...
all_trees_grown_correctly <- function(trees) {
  ...
  while (length(wrong_trees) > 0) {
    ...
    # Roman:
    #for(counttemp in seq(along=trees[wrong_trees]))
    #  trees[wrong_trees][[counttemp]]$grow(replace = TRUE)
...
get_oob_AUC <- function(trees) {
  ...
  for (curr_oob in unique_oob_ids) {
    ...
    # Roman:
    # predicted_probs <- matrix(nrow=2, ncol=length(trees_same_oob))
    # for(counttemp in seq(along=trees_same_oob)) {
    #  predicted_probs[,counttemp] <- trees[[trees_same_oob[counttemp]]]$predict(curr_oob_feas)
    # }
...
eval_fw_approach <- function(path = './compstudy_code_and_results/data/BLCA.Rda', frac_train = 0.75,
  ...
  # cols_to_keep_ex           <- colnames(train_test_bwm$Train$data)[seq(from = 1, to = length(colnames(train_test_bwm$Train$data)), by = 50)] # Roman: 81875 ersetzt durch length(colnames(train_test_bwm$Train$data))
  ...
  for (j_ in 1:amount_train_folds) {
    ...
    removeind <- which(sapply(curr_fold, function(x) sum(is.na(x)) == nrow(curr_fold)))
    if (length(removeind) > 0)
      curr_fold <- curr_fold[,-removeind]
    ...
    fold_RF <- simpleRF(formula           = formula_all, 
                        data              = curr_fold, 
                        num_trees         = 250,    # Same settings as for 'rfSCR' ! 500 !
                        mtry              = NULL, 
                        min_node_size     = 1,
    ...
    # Roman:
    #for(counttemp in seq(along=fold_RF))
    #  fold_RF[[counttemp]]$grow(replace = TRUE)
  ...
  for(l in 1:length(Forest)) {
    AUC_weight[[l]] <- get_oob_AUC(trees = Forest[[l]])
  }
  ...
  brier <- mean(((1 - probs_class_1_) - as.numeric(levels(train_test_bwm$Test$data$ytarget))[train_test_bwm$Test$data$ytarget]) ^ 2)
