...
source("./compstudy_code_and_results/code/functions/fulldata_approach_functions.R")
  ...
  for (curr_train_pattern in c(1, 2, 3, 4, 5)) {
    for (curr_test_pattern in c(1, 2, 3, 4)) {
      for (curr_repetition in c(1, 2, 3, 4, 5)) {
        ...
        curr_res <- tryCatch(eval_fd_approach(path               = curr_path, 
                                              frac_train         = 0.75, 
        ...
        curr_res$approach   <- 'Fulldata'
        
        # Add the results of the setting to 'BW_res' & save it
        FD_res <- rbind(FD_res, curr_res)
        write.csv(FD_res, './compstudy_code_and_results/results/fd_approach/FD_Eval.csv')
      }
    }
  }
  
  cat(paste("Dataset:", curr_path), "\n")
