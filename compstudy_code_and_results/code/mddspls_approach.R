...
source("./compstudy_code_and_results/code/functions/mddspls_approach_functions.R")

# [1] Run the experiments                                                    ----
# 1-1 Initalize a empty DF to store the results
mddspls_res <- data.frame()
...
for (curr_train_pattern in seq(from = args[2], to = 5, by = 1)) {
  ...
  for (curr_test_pattern in seq(from = start_value_test, to = 4, by = 1)) {
    ...
    for (curr_repetition in seq(from = start_value_rep, to = 5, by = 1)) {
      ...
      curr_res$approach   <- 'mdd-sPLS'
      
      # Add the results of the setting to 'BW_res' & save it
      mddspls_res <- rbind(mddspls_res, curr_res)
      write.csv(mddspls_res, paste0('./compstudy_code_and_results/results/mddspls_approach/MDDSPLS_Eval_', args[1], '_', args[2], '_', args[3], '_', args[4], '.csv'))
      rm(curr_res)
      gc()
    }
